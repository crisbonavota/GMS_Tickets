...
import { KeyValuePair, Update } from "../../../api/types";
import FormConditionalFields from "./FormConditionalFields";
import { generateDinamicYupSchema } from "../../../utils/forms";

// TODO: update formik impl to hooks one

export interface EditModalProps {
    update: Update;
}

export function EditButton({ update }: EditModalProps) {
    ...
    const editUpdateMutation = useMutation(
            ...
            ),
        {
            ...
            onError: (err: any) => {
                toast({
                    title: "Error updating the element, try again later",
...
const renderInitialValues = (initialValues: KeyValuePair, update: Update) => {
    ...
    if (updateTypesIds.periodUpdateTypes.includes(update.updateType.id))
        initialValues.endDate = update.endDate
            ? new Date(update.endDate).toISOString().split("T")[0]
            : "";

    if (updateTypesIds.dateChangeUpdateTypes.includes(update.updateType.id))
        initialValues.newDate = update.newDate
            ? new Date(update.newDate).toISOString().split("T")[0]
            : "";
